  ...
  try {
    ...
    try {
      ...
      if (securityContext.tokenAge > maxTokenAge) {
        console.warn(`⚠️ Old token used (${securityContext.tokenAge}ms old) from IP: ${clientIP}`);
        res.status(401).json({
    ...
    if (!user) {
      trackFailedAttempt(clientIP);
      console.warn(`🚫 User not found for token from IP: ${clientIP}, UserID: ${decoded.userId}`);
      res.status(401).json({
    ...
    if ('isActive' in user && !user.isActive) {
      trackFailedAttempt(clientIP);
      console.warn(`🚫 Inactive user attempted access from IP: ${clientIP}, UserID: ${user.id}`);
      res.status(401).json({
    ...
    console.log(`✅ Auth success - User: ${user.id}, IP: ${clientIP}, Time: ${processingTime}ms`);
  ...
  try {
    ...
    try {
      ...
      if (user && ('isActive' in user ? user.isActive : true)) {
        ...
        console.log(`ℹ️ Optional auth success - User: ${user.id}, IP: ${clientIP}`);
      }
    } catch (error) {
...
export const requireEmailVerification = (
  ...
  if (!req.user.emailVerified) {
    console.warn(`🚫 Unverified email access attempt - User: ${req.user.id}, IP: ${req.ip}`);
    res.status(403).json({
...
export const getSecurityStats = () => {
  const blocked = Array.from(suspiciousActivity.entries())
    .filter(([_, activity]) => activity.blocked).length;
